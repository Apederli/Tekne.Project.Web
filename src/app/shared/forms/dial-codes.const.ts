 ...
 *
 * Kapsam: tam dünya listesi (BM üyeleri + TW/HK/MO/PS). Bilinçli dışarıda:
 * Kosova (resmî ISO kodu yok — bayrak/ad üretilemiyor) ve Vatikan (fiilen
 * +39). Bağımlı bölgeler (PR, GP, RE…) taşınmıyor. Yeni giriş gerektiğinde
 * satır eklemek yeterli — ad/bayrak/sıralama `iso2`'den türetiliyor.
 *
 * Sıralı DEĞİL: görünen sıra `DIAL_CODES_SORTED`'da kurulur (TR başta,
 * kalanlar Türkçe ada göre). Bazı kodlar paylaşımlı olabilir (+1, +7 gibi) —
 * model yalnızca kodu sakladığı için bu bir sorun değil; ekranda ülke
 * seçimi bileşen içi durumda tutulur.
 */
export const DIAL_CODES: DialCodeOption[] = [
  ...
  // Arap pazarı: Körfez ülkeleri + Türkiye'ye yoğun turist gönderenler.
...
/** Combobox'ta gösterilen liste — TR hep başta, kalanlar Türkçe ada göre sıralı. */
export const DIAL_CODES_SORTED: DialCodeOption[] = [
  TURKEY_DIAL_CODE,
  ...DIAL_CODES.filter((c) => c !== TURKEY_DIAL_CODE).sort((a, b) =>
